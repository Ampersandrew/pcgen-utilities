...
my %cl_options = (
	...
	convert		=> q{},	# Activate a standard conversion
...
if ( $cl_options{convert} ) {
	...
	#		$conversion_enable{'ALL:PRERACE needs a ,'} = 1;
		...
		#  }
			...
			$conversion_enable{'CLASSSPELL conversion to SPELL'} = 1;
		}
		elsif ( $cl_options{convert} eq 'classskill' ) {
			$conversion_enable{'CLASSSKILL conversion to CLASS'} = 1;
		}
		elsif ( $cl_options{convert} eq 'notready' ) {
			$conversion_enable{'ALL:BONUS:MOVE conversion'} = 1;
		}
			...
			|| $cl_options{convert} eq 'ml21' )
		{
			...
			$error_message .= "\nUnknown conversion option: $cl_options{convert}\n";
			$cl_options{help} = 1;
		}
}
...
if ($cl_options{input_path}) {
	...
	if ( $cl_options{output_path} && !-d $cl_options{output_path} ) {
		$error_message = "\nThe directory $cl_options{output_path} does not exist.";
